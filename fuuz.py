...
|_____|   '.__.'_/ `.__.'  [_____] "
    ...
    def fuzzer_request(self, payload):
        ...
        selected_status_code = [int(selected_status_code) for selected_status_code in status_code.get_status_codes()]
        ...
        if (not self.content_reader is None and self.content_reader in response_content) and (response_status_code in selected_status_code):
            print(f'\r{payload.ljust(30)}           [Status: {response_status_code}, Size: {response_size}, Duration: {response_time}]')
        
        elif response_status_code in selected_status_code and self.content_reader is None:
            print(f'\r{payload.ljust(30)}           [Status: {response_status_code}, Size: {response_size}, Duration: {response_time}]')
            
        elif (not self.content_length is None and response_size >= self.content_length) and (response_status_code in selected_status_code):
            print(f'\r{payload.ljust(30)}           [Status: {response_status_code}, Size: {response_size}, Duration: {response_time}]')
